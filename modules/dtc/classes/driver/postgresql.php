...
class Driver_Postgresql extends DTCDriver
{

	public function insert($table, array $values, $primary_column, $primary_autoincrement)
	{
		if ($primary_autoincrement)
		{
			unset($values[$primary_column]);
		}
		
		return $this->db->query(Database::INSERT, 'insert into ' . DTCQuery::quote_table($table) . '(' . implode(',', array_keys($values)) . ') values(' . implode(',', array_values($values)) . ')', true, array($table));
	}
	...
	public function delete($table, $condition, $primary_column, $used_tables = array())
	{
		...
		{
			return $this->db->query(Database::DELETE, 'delete from ' . DTCQuery::quote_table($table) . ' where ' . DTCQuery::quote_identifier($primary_column) . ' in (' . implode(',', $condition) . ')', true, $used_tables);
		}

		return $this->db->query(Database::DELETE, 'delete from ' . DTCQuery::quote_table($table) . ' where ' . DTCQuery::quote_identifier($primary_column) . ' = ' . $condition, true, array($table));
	}

	public function select(DTCQuery $query, $used_tables)
	{
		return $this->db->query(Database::SELECT, $query->get_query(), true, $used_tables)->as_array();
	}
	...
	public function update($table, array $values, $condition, $primary_column)
	{
		...
		return $this->db->query(Database::UPDATE, 'update ' . DTCQuery::quote_table($table) . ' set ' . $query_set_segment . ' where ' . $primary_column . ' = ' . $condition, true, array($table));
	}
	...
	public function quote($value)
	{
		switch(gettype($value))
		{
			...
			default:
			case 'string':
				if (($value = pg_escape_string((string) $value)) === false)
	...
	{
		$query->fields('count(*) as count')->order_by()->limit()->unset_field();
	...
	public function validate_rule($parameters)
	{
		...
		{
			$column_rules['decimal'] = array_values($decimal);
		}
